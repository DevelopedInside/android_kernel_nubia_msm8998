...
static struct attribute *nubia_attrs[] = {
	attrify(read_rt2),
	attrify(read_rt3),
	attrify(ic_detect),
	NULL,
...
static struct attribute_group nubia_attr_group = {
	.attrs = nubia_attrs,
};
#endif
...
#ifdef NUBIA_TOUCH_SYNAPTICS
	retval = sysfs_create_group(&rmi4_data->pdev->dev.parent->kobj, &nubia_attr_group);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to create sysfs attributes\n",
				__func__);
		goto nubia_exit_attributes;
	}
