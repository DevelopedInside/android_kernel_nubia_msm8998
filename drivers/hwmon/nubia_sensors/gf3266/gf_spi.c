...
#define	GF_INPUT_NAME	    "gf3266"	/*"goodix_fp" */
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_ENABLE_POWER:
		...
		gf_power_control(gf_dev, true);
		break;
	case GF_IOC_DISABLE_POWER:
		pr_debug("%s GF_IOC_DISABLE_POWER\n", __func__);
		gf_power_control(gf_dev, false);
		break;
...
int gf_power_init(struct gf_dev *dev, bool on)
{
	int rc;
    ...
    if (on) {
		dev->vdd = regulator_get(&dev->spi->dev, "gf_vdd");
		if (IS_ERR(dev->vdd)) {
			rc = PTR_ERR(dev->vdd);
		...
		if (!IS_ERR_OR_NULL(dev->vdd)) {
			if (regulator_count_voltages(dev->vdd) > 0) {
				rc = regulator_set_voltage(dev->vdd, GF_VDD_MIN_UV, GF_VDD_MAX_UV);
				if (rc) {
        ...
        if (regulator_count_voltages(dev->vdd) > 0) {
            regulator_set_voltage(dev->vdd, 0, GF_VDD_MAX_UV);
        }
...
#endif
{
	...
	gf_dev->device_available = 0;
	gf_dev->fb_black = 0;
	gf_dev->power_on = false;

	if (gf_parse_dts(gf_dev))
		goto error_hw;
	status = gf_power_init(gf_dev, true);
	if (status) {
	...
	if (status == 0) {
		...
		if (gf_dev->input == NULL) {
			SENSOR_LOG_ERROR("failed to allocate input device\n");
			status = -ENOMEM;
			goto error_dev;
		}
		...
		if (status) {
			SENSOR_LOG_ERROR("failed to register input device\n");
			goto error_input;
		}
	}
#ifdef AP_CONTROL_CLK
